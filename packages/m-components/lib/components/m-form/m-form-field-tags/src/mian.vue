 ...
 * @LastEditTime: 2023-06-25 16:32:45
 * @LastEditors: squanchy squanchy@yeah.net
 * @Description: type 的设置 可以写在一个组件里面，可以单独拉出来，看你这个组件复杂不，复杂的话一个模块一个type
 * @FilePath: \zs-ui-vue2\packages\m-components\lib\components\m-form-item-tags\src\mian.vue
-->
<template>
  ...
  watch: {
    value: {
      ...
      handler: function (value) {
        // 延迟到组件创建完毕后再进行
        this.$nextTick(() => {
          if ([null, undefined].includes(value) || !Array.isArray(value)) {
            this.previousValue = [];
          } else if (JSON.stringify(value) !== JSON.stringify(this.previseValue)) {

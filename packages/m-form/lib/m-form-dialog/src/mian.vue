 ...
 * @LastEditTime: 2022-04-16 18:06:01
  ...
  methods: {
    ...
    async openDialog({ type = 'create', dialogData = {}}) {
      const promise = new Promise((resolve, reject) => {
        ...
        let { originFormItems, originFormData } = this.dialogOption.generateFormItems({ that: this, openDialogData: dialogData })
    ...
    submit(extraData) {
      this.$refs[this.dialogOption.ref]
        ...
        .then(async(res) => {
          ...
          // eslint-disable-next-line prefer-const
          let { params, api, message, resolveData = {}} = submitInterceptor(this.formOriginData, this.formData, extraData)
          if (api) {
